...
from sklearn.metrics import accuracy_score,f1_score,average_precision_score,recall_score, confusion_matrix, roc_curve, auc, classification_report
import random
...
def binaryf1(pred,counts):
    return f1_score(counts,pred)
...
def precision_value(pred, target):
    assert pred.shape[0] == len(target)
    precision = average_precision_score(target, pred)
    return precision

def recall_value(pred, target):
    assert pred.shape[0] == len(target)
    recall = recall_score(target, pred)
    return recall

def tpr_fpr_tnr_fnr(pred, target):
    assert pred.shape[0] == len(target)
...
def class_report(pred, target):
    report = classification_report(target, pred, digits=4)
    return report
