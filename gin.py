...
class GIN(torch.nn.Module):
    def __init__(self, config, ):
        ...
        #self.linear=torch.nn.Linear(config["gcn_hidden_dim"] * config["gcn_graph_num_layers"], config["prompt_output_dim"])
    ...
    def forward(self, graph, graph_len, prompt=None):
        graph_output = graph.ndata["feature"]
        
        if prompt is not None:
            graph_output = prompt.add(graph_output)
        xs = []
