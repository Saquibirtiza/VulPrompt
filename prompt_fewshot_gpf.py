...
from graph_prompt import SimplePrompt, GPFplusAtt
...
from utils import anneal_fn, get_enc_len, pretrain_load_data, get_linear_schedule_with_warmup,\
    ...
    distance2center,center_embedding, tpr_fpr_tnr_fnr, roc_auc, binaryf1, precision_value, recall_value, class_report
'''from cnn import CNN
from rnn import RNN
from txl import TXL
...
train_config = {
    ...
    # "graph_dir": "../convertor/ffmpeg/raw",
    "save_data_dir": "../dataffmpeg/ENZYMESPreTrain",
    "save_model_dir": "../dumpsffmpeg/debug",
    "save_pretrain_model_dir": "../dumps100sent2vec/ENZYMESPreTrain/GIN",
    # "graphslabel_dir":"../convertor/ffmpeg/ENZYMES_graph_labels.txt",
    "downstream_graph_dir": "../dataffmpeg/debug/graphs",
    # "downstream_save_data_dir": "../dataffmpeg/debug",
    "downstream_save_model_dir": "./dumpsffmpeg/ENZYMESGraphClassification/Prompt/GIN-FEATURE-WEIGHTED-SUM/5train5val100task",
    # "downstream_graphslabel_dir":"../data100sent2vec/debug/graphs",
    "temperature": 0.01,
    ...
    "save_fewshot_dir": "../dataffmpeg/ENZYMESGraphClassification/fewshot",
...
def train(pretrain_model, model, optimizer, scheduler, data_type, data_loader, device, config, epoch, label_num, logger=None, writer=None):
    ...
    for batch_id, batch in enumerate(data_loader):
        ...
        x,embedding=pretrain_model(graph,graph_len,prompt)
        # graph_label_onehot=l2onehot(graph_label)
        embedding = embedding*train_config["scalar"]
        ...
        if logger and (batch_id % config["print_every"] == 0 or batch_id == epoch_step - 1):
            logger.info(
                "epoch: {:0>3d}/{:0>3d}\tdata_type: {:<5s}\tbatch: {:0>5d}/{:0>5d}\tbp loss: {:0>5.8f}".format(
    ...
    if logger:
        logger.info("epoch: {:0>3d}/{:0>3d}\tdata_type: {:<5s}\tbp loss: {:0>5.8f}\tmean_acc: {:0>1.3f}".format(
            epoch, config["epochs"], data_type, mean_bp_loss,mean_acc))
    gc.collect()
    return mean_reg_loss, mean_bp_loss, total_time,mean_acc,c_embedding


def evaluate(pretrain_model, prompt, data_type, data_loader, device, config, epoch, c_embedding, label_num, debug=False,logger=None, writer=None):
    epoch_step = len(data_loader)
    total_reg_loss = 0
    total_step = config["epochs"] * epoch_step
    total_bp_loss = 0
    ...
    with torch.no_grad():
        for batch_id, batch in enumerate(data_loader):
            ...
            precision = precision_value(eval_pred, eval_graph_label)
            recall = recall_value(eval_pred, eval_graph_label)
            f1 = binaryf1(eval_pred, eval_graph_label)
            macrof=macrof1(eval_pred,eval_graph_label)
            tpr, fpr, tnr, fnr = tpr_fpr_tnr_fnr(eval_pred, eval_graph_label)
            auc = roc_auc(eval_pred, eval_graph_label)
            weightf=weightf1(eval_pred,eval_graph_label)
            report = class_report(eval_pred,eval_graph_label)
            total_acc+=acc
            ...
            if logger and batch_id == epoch_step - 1:
                logger.info(
                    "epoch: {:0>3d}/{:0>3d}\tdata_type: {:<5s}\tbatch: {:0>5d}/{:0>5d}\tbp loss: {:0>5.8f}\taccuracy: {:0>1.3f}".format(
                        epoch, config["epochs"], data_type, batch_id, epoch_step, bp_loss_item,accuracy))
        mean_reg_loss = total_reg_loss / total_cnt
        mean_bp_loss = total_bp_loss / total_cnt
        mean_acc=total_acc/batchcnt
        mean_precision = total_precision / batchcnt
        mean_recall = total_recall / batchcnt
        mean_f1 = total_f1 / batchcnt
        mean_macrof=total_macrof/batchcnt
        mean_tpr, mean_fpr, mean_tnr, mean_fnr = total_tpr / batchcnt, total_fpr / batchcnt, \
                                                 total_tnr / batchcnt, total_fnr / batchcnt
        mean_auc = total_auc / batchcnt
        mean_weighted=total_weighted/batchcnt
        if writer:
            writer.add_scalar("%s/REG-%s-epoch" % (data_type, config["reg_loss"]), mean_reg_loss, epoch)
            writer.add_scalar("%s/BP-%s-epoch" % (data_type, config["bp_loss"]), mean_bp_loss, epoch)
        if logger:
            logger.info("epoch: {:0>3d}/{:0>3d}\tdata_type: {:<5s}\tbp loss: {:0>5.8f}\tacc:{:0>1.3f}".format(
                epoch, config["epochs"], data_type, mean_bp_loss,mean_acc))

    gc.collect()
    return mean_reg_loss, mean_bp_loss, evaluate_results, total_time,mean_acc, mean_precision, mean_recall, mean_f1, mean_macrof, mean_tpr, mean_fpr, mean_tnr, mean_fnr, mean_auc, mean_weighted,c_embedding, report
...
if __name__ == "__main__":
    ...
    f1 = list()
    macroF = list()
    tpr, fpr, tnr, fnr = list(), list(), list(), list()
    auc = list()
    weightedF = list()
    ...
    for count in range(train_config["few_shot_tasknum"]):
        ...
        prompt  = GPFplusAtt(train_config["emb_dim"], 5).to(device)
        pre_train_model.load_state_dict(torch.load(os.path.join(save_pretrain_model_dir, 'best.pt')))
        model = model.to(device)
        logger.info(model)
        logger.info("num of parameters: %d" % (sum(p.numel() for p in prompt.parameters() if p.requires_grad)))
        ...
        if train_config["update_pretrain"]:
            ...
            optimizer = torch.optim.AdamW([{"params": pre_train_model.g_net[2].apply_func[2].parameters()},{"params": prompt.parameters()}], lr=train_config["lr"],
                                          weight_decay=train_config["weight_decay"], amsgrad=True)
        ...
        for epoch in range(train_config["epochs"]):
            for data_type, data_loader in data_loaders.items():
                ...
                if data_type == "train":
                    mean_reg_loss, mean_bp_loss, _time, accfold,c_embedding = train(pre_train_model, prompt, optimizer, scheduler, data_type, data_loader, device,
                    ...
                    mean_reg_loss, mean_bp_loss, evaluate_results, _time,accfold, precisionfold, recallfold, f1fold, macroFfold, tprfold, fprfold, tnrfold, fnrfold, aucfold, weightedFfold,c_embedding, report = evaluate(pre_train_model, prompt, data_type, data_loader, device,
        ...
        for data_type, data_loader in data_loaders.items():
            mean_reg_loss, mean_bp_loss, evaluate_results, _time, acctest, precisiontest, recalltest, f1test, macroFtest, tprtest, fprtest, tnrtest, fnrtest, auctest, weightedFtest,c_embedding, report = evaluate(
                pre_train_model, prompt, data_type, data_loader, device,
                train_config, epoch, best_c_embedding, train_config["test_label_num"], debug=False, logger=logger,
                writer=writer)

        acc.append(acctest)
        precision.append(precisiontest)
        recall.append(recalltest)
        f1.append(f1test)
        ...
        print('precision for 10fold: ', precision)
        print('recall for 10fold: ', recall)
        print('f1 for 10fold: ', f1)
        print('macroF for 10fold: ', macroF)
        print('weightedF for 10fold: ', weightedF)
        print('tpr for 10fold: ', tpr)
        print('fpr for 10fold: ', fpr)
        print('tnr for 10fold: ', tnr)
        print('fnr for 10fold: ', fnr)
        print('auc for 10fold: ', auc)
        ...
        print('precision mean: ', np.mean(precision), 'precision std: ', np.std(precision))
        print('recall mean: ', np.mean(recall), 'recall std: ', np.std(recall))
        print('f1 mean: ', np.mean(f1), 'f1 std: ', np.std(f1))
        ...
        print('tpr mean: ', np.mean(tpr), 'tpr std: ', np.std(tpr))
        print('fpr mean: ', np.mean(fpr), 'fpr std: ', np.std(fpr))
        print('tnr mean: ', np.mean(tnr), 'tnr std: ', np.std(tnr))
        print('fnr mean: ', np.mean(fnr), 'fnr std: ', np.std(fnr))
